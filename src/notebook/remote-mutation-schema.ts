...
export interface RemoteMutationJournalV1 {
  ...
  result_category: "prepared" | "possibly-dispatched" | "reconciled-zero" | "reconciled-one" | "reconciled-many" | "committed";
  next_action: string;
}
...
const OPTIONAL_KEYS = ["binding_id", "session_key"] as const;
...
export function remoteMutationResultCategory(state: RemoteMutationJournalState, candidateCount: number): RemoteMutationJournalV1["result_category"] {
...
export function remoteMutationNextAction(state: RemoteMutationJournalState, candidateCount: number): string {
