...
addressed (story 1.8, PJAN-110): the undeclared `systemd.*` keys are now load-bearing exactly as predicted, and the systemd observer USES the gap rather than closing it. `service_manifest` derives the set of DECLARED registry keys from the contract's own `writable_fields` (`agents.{agent_id}.systemd.<leaf>`), so a key on a live row that no authority declares -- `checkpoint_timer`, `consumer_unit`, `cron_tick_timer`, `artifact_bridge_timer`, `watchdog_timer` -- is reported `registry-retired-key:<key>` on the topology leaf. Declaring an owner for one would make it canonical, which is the opposite of what the live fleet needs; the two keys the fleet DOES own (`gateway_unit`, `heartbeat_timer`) are declared and are the two leaves the observer files under. `telegram.provisioning_status` and `slack.provisioning_status` were declared for this story (they are what the desired gateway state is derived from) and the validator now refuses a `service_manifest` whose `status_field` resolves to a path no authority writes. The rest of the DW-1 list -- `hindsight.*`, `reporting.*`, `internal_role_name`, the remaining `slack.*`/`telegram.*` identity fields, `hermes.codex_home`, `gateways.bloodbank.legacy_profile_consumers` -- is still undeclared and still unread.
...
addressed (story 1.8, PJAN-110): `units.*` is no longer a namespace nothing reads. The three per-agent unit leaves and the fleet-shared gateway leaf are now the fields the systemd observer files its observations under, `ownerOf` resolves each to `hermes-fleet-provisioner` through the `systemd_lifecycle` authority that already declared them, and `service_manifest` says how each is PROVEN -- so the leaves name units on a real manager rather than a shape the contract invented. `scaffold.*` was answered the same way by story 1.6.
...
addressed (story 1.8, PJAN-110): the systemd observer does not consume `expected_units` at all. It re-derives each unit from `service_model.per_agent` by KEY (`gateway_unit`, `heartbeat_timer`, `heartbeat_service`), so which pattern produced which name is never lost: the gateway leaf is filed under the gateway pattern's field, the two heartbeat leaves under theirs, and `agents[].systemd.topology.expected` carries the derived triple beside `installed`/`missing`/`extra`. The inventory's own flat `expected_units` list is unchanged and remains what DW-29 describes.
...
status: open
addressed (both halves): The TYPE half -- `FleetInventoryRow.classification` is `FleetFieldValue<FleetClassificationId>`, so a typo no longer compiles. The RESOLUTION half -- `declaredRowClass` (`src/fleet/inventory.ts`) promotes a row to `retired` or `intentionally_unmanaged` when the contract has recorded an entry naming it by a CONCRETE `source: agents.<agent_id>`, compared as a whole string so `agents.alpha-pm` cannot also claim `agents.alpha-pm-2`. The `{agent_id}` placeholder form deliberately claims nobody: it is the shape of a path, not an instance, and honouring it would let one entry sweep the whole fleet into a class no operator ruled on. `managed_agent` remains the default and a malformed row is never promoted.

  CORRECTION, same story, after review: the first cut ALSO matched the entry's `participants` list, on the reasoning that it was "the convention `matchException` already uses" and so one shape for an operator to learn. That was wrong, and measured wrong -- a contract permitting two agents to share one `profile_name` reclassified BOTH participating rows to `intentionally_unmanaged`. The two spellings are rulings about two different SCOPES: `participants` permits one identity-conflict GROUP (a statement about one field path shared by named claimants), while a row class is a statement about the whole row. Nothing broke at the time only because both roads happened to land in the `exception` member bucket; the damage was deferred to stories 1.11 and 1.15, where `row.classification` becomes an input to what a reconciliation plan skips. A convergence engine declining to touch a row because someone permitted a profile-name collision is exactly the unearned exemption this epic exists to prevent, and one that would have been very hard to trace back. The `participants` arm is removed; `matchException` still reads it, for the group ruling it was written for. Rationale is in `entryClaimsAgent`'s doc comment.
...
addressed (story 1.8, PJAN-110): `systemd`/`unit_topology` is REMOVED from `health_policy.deferred_capabilities`: the systemd observer samples the user manager itself over a declared stabilization window and proves each agent's canonical unit set against the capability its registry row declares, so no derived expectation is carried as evidence any more. TWO observer-less halves remain -- `live_process`/`process_attribution` (story 1.9) and `bloodbank`/`routing_liveness` (story 1.10) -- and they are now the only entries in the list. `health.complete` on this build is about seven of the nine domains rather than six.
...
status: open
note (story 1.8, PJAN-110): unchanged. The systemd observer proves the fleet-shared Bloodbank gateway is INSTALLED and healthy on the user manager (`systemd.shared-gateway`), which is a precondition for routing and not routing readiness: it reads no `routing_prerequisites` constraint, evaluates no target, and grants nothing. `lifecycle.capability_readiness` still reports `unproven`, and story 1.10 still owns the resolver.
...
status: open
addressed in part (story 1.8, PJAN-110): the systemd half is now observed. A gateway that is enabled, active, running and stable over the declared window is a real reading of "installed and running", and a deferred or drifted one is reported as such on `units.hermes-{agent_id}-gateway.service` -- so the lifecycle's `observed_state` is no longer derived from a domain that observed nothing. What remains unreachable is the half that needs a PROCESS: nothing in this build reads the process table, so a unit that systemd believes is active is still not proof that a Hermes process is attributable to that row (story 1.9), and `capability_readiness` still needs story 1.10.
...
### DW-81: a field-level conflict ruling silently granted a row-level lifecycle exemption.
origin: review-deferred story-1.5 review pass 1
location: src/fleet/inventory.ts (entryClaimsAgent / declaredRowClass)
source_spec: `spec-1-5-make-partial-health-truthful-and-actionable.md`
severity: medium
note: Filed as DW-81 rather than the DW-77 the coordinator asked for; 77 was already taken by the import-cycle entry landed earlier in the same pass.
reason: `declaredRowClass` matched a classification entry to a row by EITHER the entry's `participants` list or a concrete `source: agents.<agent_id>`. `participants` was chosen for operator convenience -- "the convention `matchException` already uses" -- and that was the wrong instinct, because the two readings are about different SCOPES. `matchException` reads `participants` to permit one identity-conflict GROUP: a statement about one field path shared by named claimants. Reading the same list in `declaredRowClass` turned it into a statement about the whole ROW. MEASURED with a two-agent fixture sharing one `profile_name` and a contract permitting it: both participating rows came back `classification: intentionally_unmanaged`, which nobody wrote.

Nothing broke at the time, because both roads landed in the `exception` member bucket and the answer was the same either way. The damage was DEFERRED, which is what makes it worth an entry rather than a footnote: `row.classification` is inventory output that stories 1.11 and 1.15 consume, a row marked `intentionally_unmanaged` is one a reconciliation plan should plausibly skip, and permitting a PROFILE-NAME collision must not exempt that row from SCAFFOLD convergence. A convergence engine declining to touch a row for a reason recorded three stories earlier, in a different command, about a different field, is close to untraceable.
status: closed
...
status: open
addressed in part (story 1.8, PJAN-110): the profile observer's text scan is unchanged, but the unit STATE it could never see is now observed elsewhere. The systemd observer asks the manager itself (`systemctl --user show -p Environment,...`), so a unit's `HERMES_HOME` is read from the manager's own parsed value rather than from a line in a fragment -- drop-ins included, `EnvironmentFile=` still excluded by design -- and every `hermes-*` unit the manager loads is classified, with a unit whose home names a profile-root directory reported `profile-correlated`. The two readings are deliberately separate: DW-93's scan answers "how many unit FILES mention this profile directory" for the extras sweep, and the systemd observer answers "what is this unit's state and home right now". A unit that names a profile only through an `EnvironmentFile=` is still invisible to both.
...
source_spec: `spec-1-8-prove-canonical-systemd-topology-and-service-health.md`
severity: low
reason: `platform-enablement-inherited:<platform>` fired whenever a deferred platform's `platforms.<platform>.enabled` was not pinned `false` in the profile's `config.delta.yaml`, without ever opening the fleet base. The deferral argued that reading the base would make the item's truth depend on a file the delta is supposed to override.
status: resolved (story 1.8 matrix-test audit, PJAN-110)
resolution: The matrix row's INPUT is "delta `{}` (base has `platforms.telegram.enabled: true`)" and its expected output names exactly one item -- so the row is about inheritance FROM THE BASE, and the delta-only rule could not discriminate it: a delta-empty agent was flagged for every deferred platform, including ones nothing enables anywhere, saying "inherited" about an enablement that does not exist. The observer now reads the fleet base once per run (`readBaseEnablement`, the same `HERMES_HOME/config.yaml` the renderer merges every delta over) and reports the EFFECTIVE enablement: the delta's pin when it has one, the base's value when it does not, and an item only when that resolves to `true`. The "silently stops requiring the pin" cost the deferral worried about does not arise: a base that flips to `true` produces the item on every delta-empty profile in the same run, because the base is read on every run rather than assumed. `tests/fleet-systemd-regressions.mjs` drives the base itself -- base-enabled, base-disabled, and delta-pinned-on against a disabled base -- so the rule now fails in both directions.
