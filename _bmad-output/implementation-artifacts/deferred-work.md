...
origin: spec-deferred story-1.8 (PJAN-110)
location: src/fleet/systemd.ts (readDeltaEnablement)
source_spec: `spec-1-8-prove-canonical-systemd-topology-and-service-health.md`
severity: low
reason: `platform-enablement-inherited:<platform>` fires whenever a deferred platform's `platforms.<platform>.enabled` is not pinned `false` in the profile's `config.delta.yaml`. That is exactly what `channel-transaction.py` writes on deferral and exactly what the live fleet's base enables (`~/.hermes/config.yaml` sets `platforms.telegram.enabled: true`), so the reading is correct on every fleet this contract describes. It is deliberately NOT derived from the base: reading the base would make the item's truth depend on a file the delta is supposed to override, and a base that happened to disable a platform today would silently stop requiring the pin that protects against it being enabled tomorrow. The cost is one false-ish positive shape -- a fleet whose base disables a platform gets an item asking for a pin it does not strictly need. Resolving it means deciding whether the pin is a REQUIREMENT of deferral or merely an effect of it, which is a template-contract question, not this observer's.
status: open
