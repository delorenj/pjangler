...
- `src/project/lifecycle/types.ts` -- Define every strict lifecycle input and nested output contract in Design Notes as Zod schemas plus inferred TypeScript types, including the identity checkpoint union, effect-journal rows/cursor, project/repo-link receipt union, exact Plane spine and recovery identity, BMAD baseline/config-hash evidence, normalized selections, exact runtime marker, exact fleet row, parent provider identity versus internal UUID, persisted `provisioned_at`, evidence, planned effects, observations, blocked references, nullable errors, indeterminate effects, actual public-wire output accounting, authority sources, and all row/result-wide byte/count bounds. Reject unknown lifecycle fields, invalid enums/UUIDs/URLs/paths, duplicate semantic keys/IDs, controls, and aggregate excess. Keep the outer `automation` envelope lossless and validate only its owned `lifecycle` child strictly. Emit snake_case `LifecycleResultV1`; accept camelCase shared/MCP inputs and mechanically mapped kebab-case CLI flags. Validate before serialization; allow the null-receipt constant fallback only before a durable checkpoint/mutation and rebuild every later failure from the checkpoint plus the validated durable journal prefix.
...
- `src/project/lifecycle/hermes.ts` -- Render pinned Hermes with `copier copy --skip-tasks`; do not execute its opaque task scripts. Normalize rendered `role.yaml provisioned_at` to the persisted lifecycle timestamp before ownership comparison. Implement named observed subactions for repo render, strict runtime marker, private Plane env, selection-conditional runtime seed, fail-closed secret scan, deterministic runtime commit and exact push, exact parent gitlink, host profile/CST-preserving fleet-row merge, secure symlink-aware Bloodbank interpreter proof, provider-idempotent durable consumer convergence, lifecycle-owned exact user-unit templates, and a controlled lifecycle heartbeat. Follow the global-registry/project/heartbeat/fleet lock order and hold the required locks through runtime push, parent push, verification, and timer decision. Never trust `.done-*` markers, copy global/profile secrets, run `git add -A`, install into the shared Hermes venv, overwrite an opaque fleet row, or enable the timer before both pushes verify.
...
- `src/project/lifecycle/index.ts` -- Implement static plan, explicit readiness, gated execution, and the hidden persisted-identity heartbeat. Normalize `parent_repository_name=slug`, composite SSH URLs, PM/sentinel/Bloodbank selections, deterministic Plane candidates, and checkpoint/provider-operation keys; aggregate all applicable input/path/authoritative-registry/Git/GitHub/Copier/exact-object/BMAD/Plane/PG/Hermes/host/output-spine checks before a permit or durable mutation. After full preflight, acquire locks in the global order, reread registry authority, allocate/reuse UUIDs and one timestamp, and durably fsync the journal header plus identity checkpoint before the first business mutation. Execute the exact composite phases; append/fsync intent before each request, append/fsync independent observations, then advance checkpoint before continuing. Repair mirrors from checkpoint+journal, stop forward work after failure, run the local redacted finalizer, and apply deterministic full-wire output compaction. Verify final checkpoint/journal/YAML/PG/manifest IDs/status, selections, Plane recovery identity, BMAD hashes, origin/ref/ancestry, provider identities, timestamp, PM/sentinel/fleet identities, unit bytes/states, ownership ledger, a clean lifecycle-owned index with no new unowned worktree delta, preserved pre-existing unrelated state, and pushed HEAD.
...
- `tests/project-lifecycle-core-regressions.mjs` -- Cover all I/O/authority rows, prior review rows, and all eighteen Fifth Review acceptance rows: exact/deinitialized-submodule object reads, structured CommonProject replacements, receipt-visible BMAD hashes, selection-keyed checkpoint/Plane recovery identity, journal crash/corruption recovery, cross-project registry locking, conditional runtime seeds, symlink-safe interpreters, exact units/heartbeat, deterministic Plane identifiers, canonical JSON, lossless automation, cross-host provider convergence, physical aliases, ownership drift, output-spine emergency partial receipts, runtime identity/scan/lock/OID order, tenant-private env, secret argv rejection, indeterminate outcomes, and deterministic reruns with injected providers.
...
The union and every nested owned object are strict. Project checkpoints require canonical `project_id`/`repo_id`, the resolved selections, and the live-preflight-selected immutable Plane base/source before their first fsync; repo-link checkpoints contain no project/provider/Plane/BMAD/runtime fields. All initially unknown observed fields are null, never guessed. `provisioned_at` is one injected UTC-second sample. Existing checkpoints must pass filename/content hash, schema, full key, physical identity, selection, Plane base/source, and collision checks; matching reruns reuse IDs, source, base, and timestamp byte-for-byte. A malformed/conflicting checkpoint is `REGISTRY_IDENTITY_MISMATCH`, and a source/base change is `PLANE_RECOVERY_IDENTITY_MISMATCH`; neither is overwritten. After every independently confirmed provider ID, BMAD hash set, or commit OID, append/fsync its observation then atomically advance the checkpoint cursor/document before issuing the next mutation. The checkpoint is retained after success as pjangler-owned recovery evidence, its ID/hash/cursor are mandatory in every executing receipt, and failure to create/update it stops before the next mutation. This file and its paired effect journal are the only allowed recovery persistence preceding business mutation; both are declared host effects and are never silently cleaned up.
...
Each object is strict and uses the existing row/byte bounds. Sequence numbers begin at one, increase by exactly one, and each `prev_row_sha256` equals SHA-256 of the previous canonical row bytes without LF. For each append, validate and canonicalize the complete row, issue one full-write loop under the held locks, append exactly one LF, `fsync` the journal, then atomically write/fsync/rename/fsync-directory the checkpoint with the new `{seq,row_sha256}`. That order is mandatory: a mutation intent and advanced checkpoint cursor must both be durable before its provider/child/host request issues; an observation and advanced cursor must both be durable before the next mutation issues. Never update the checkpoint ahead of the journal.

On recovery, the journal is authoritative for which mutation requests were issued and which effects were independently observed; the checkpoint is authoritative for identity, immutable selections/Plane source, and the acknowledged prefix. The header duplicates only the initial immutable identity needed to recover a crash between header fsync and initial checkpoint rename. Read the entire bounded file without following links, require exactly one LF-terminated JSON object per line, validate the header, sequence, hash chain, intent/observation pairing, unique effect IDs, and checkpoint cursor. If a valid header with no rows exists but its checkpoint is absent, reconstruct exactly the initial all-observations-null checkpoint from `initial_identity`, set the header cursor, fsync it, and continue; any row in a checkpoint-less journal is corrupt because no request was authorized. If valid rows extend beyond a behind checkpoint cursor, treat them as a crash tail, atomically advance the checkpoint to the last valid row, and continue. A fsynced intent without a terminal observation triggers all declared observers before any new mutation; append a confirmed or indeterminate observation, and never blindly reissue.

A missing journal is allowed only when no checkpoint exists; create both as above. A checkpoint with a missing/empty journal, a checkpoint cursor ahead of the valid chain, a malformed/non-LF-terminated interior row, hash mismatch, duplicate/skip, unknown schema, or conflicting effect is `EFFECT_JOURNAL_CORRUPT`. If the checkpoint cursor is zero and all provider/BMAD/commit/last-action fields remain null, block before further business mutation with no claimed business effect. If its cursor is positive, any provider/BMAD/commit/last-action field is non-null, or any valid intent exists, stop all forward mutation, derive the receipt/effects from the last valid prefix plus independently re-observable checkpoint facts, mark each unmatched or no-longer-provable issued intent indeterminate, and return `partial`/6 with `EFFECT_JOURNAL_CORRUPT`. Never truncate, overwrite, quarantine, or auto-repair corrupt bytes.
...
1. `identity.checkpoint` -- allocate or reuse `project_id`, `repo_id`, resolved selections, immutable Plane base/credential-source name, and one `provisioned_at`; create/fsync the journal header and strict checkpoint with every unknown provider/OID/BMAD observation null before any business mutation.
...
PJAN-27 performs no Bloodbank package install. Selection is the one resolved `ProjectCreateInputV1.bloodbank` boolean defined at the shared surface; CLI and MCP cannot diverge from it. When true, preflight `lstat`s lexical `${HERMES_AGENT_REPO}/.venv/bin/python`. It may be a current-uid-owned executable regular file or a symlink whose bounded chain has at most 16 links, no loop, no group/world-writable link or parent, and terminates at an executable regular file owned by current uid or root with no group/world write. The terminal interpreter may be outside the Hermes repository, as with a normal venv symlink to `/usr/bin/python3`; record and recheck every link `(dev,ino,uid,mode,target)` plus terminal `(dev,ino,uid,mode)` immediately before unit write/start. Invoke the lexical venv path, not the terminal realpath, with `-I` so venv package resolution is preserved. Broken/looping/unsafe/replaced chains are `UNSAFE_INTERPRETER`.

Both `import nats` and `importlib.metadata.version("nats-py") === "2.15.0"` must pass through that captured interpreter before the identity checkpoint or any business mutation; otherwise `MISSING_DEPENDENCY` blocks with external remediation guidance. Provisioning renders the pinned consumer only when selected, rejects unresolved placeholders, points its unit to that exact lexical interpreter, and records the package as confirmed `reused`. When false, the probe/action/consumer-file/unit/package/provider effect are all `skipped`/absent and the fleet row uses `consumer_unit:null`. Never run `uv`/`pip`, mutate the shared venv/lock, ignore probe failure, or mark dependency success from a marker.

The selected consumer uses stream `BLOODBANK` and durable name `pjangler-${agent_id}`. Its strict desired durable config is `{ack_policy:"explicit",deliver_policy:"all",durable_name,filter_subject:"bloodbank.${agent_id}",max_ack_pending:1,replay_policy:"instant"}`. Preflight may report NATS unreachability as a bounded warning because the process retries, but the first live process convergence performs `consumer.info`; absent creates exactly that config, present reuses only parsed equality, and mismatch is `BLOODBANK_CONSUMER_CONFLICT`. Timeout/name-exists invokes `consumer.info` recovery and never a blind second create.
...
ExecStart="${hermes_bin}" gateway --profile "${profile_name}"
Restart=on-failure
RestartSec=5s
NoNewPrivileges=yes
PrivateTmp=yes
...
The hidden `lifecycle heartbeat` adapter accepts only the exact IDs shown, reopens the strict checkpoint/journal/mirrors, and acquires global-registry → project → heartbeat locks. It stages only ownership-proved changed runtime leaves under `memories/MEMORY.md`, `memories/USER.md`, and `decisions/*.md`; any symlink, unowned path, pre-existing staged entry, config drift, or other delta blocks. It uses the persisted author/committer identity and `provisioned_at`, message `chore(pjangler): checkpoint ${agent_id} runtime`, exact runtime URL and `refs/heads/main:refs/heads/main`, then updates the parent gitlink with message `chore(pjangler): checkpoint ${slug} lifecycle` and exact parent refspec. Every Git child uses the controlled Git boundary; the heartbeat never invokes shell, `git add -A`, `git push origin HEAD`, force, wall-clock dates, or configured remote selection. It appends intents/observations and advances checkpoint/mirrors after each observed push. Failure leaves the timer enabled, returns nonzero, and records partial/indeterminate state for the identical next tick to recover; it never retries blindly in the same tick.
...
| `http-injection` | local/worktree `http.*.extraHeader`, cookie file/save, HTTP proxy, any `credential.*`, or inherited header/proxy/config-count/GCM/askpass variables | block local config before network; sanitized control contains only the injected private auth boundary and explicit empty HTTP/credential controls |
