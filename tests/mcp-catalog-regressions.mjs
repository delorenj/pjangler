...
for (const tool of [
  ...
  // `server\.` is kept, not dropped. Widening the pattern to a bare
  // `registerTool(` would cost these tools their guarantee that the call is
  // made ON THE SERVER -- any `registerTool("name"` text anywhere in the file
  // would then satisfy it.
  assert.match(source, new RegExp(`server\\.registerTool\\(\\s*["']${tool}["']`), `${tool} must be registered on the server`);
}
