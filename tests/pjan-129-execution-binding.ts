...
const actor=(id:string)=>({role:id==='pm'?'pm':'operator',native_user_id:id,key_ref:`op://DeLoSecrets/${id}/key`,runtime_id:'host',runtime:{adapter:'systemd',unit_prefix:id,planner_argv:['hermes','chat']}});
const manifest:any={project_id:'test',ticket_provider:{type:'plane',workspace:'test',board_id:'board'},execution:{mode:'managed',policy_version:2,skill_version:'sha256',pilot_bundle_sha256:'a'.repeat(64),momo_bundle_sha256:'b'.repeat(64),pm_actor:'pm',controller_actor:'repair',actors:{pm:actor('pm'),repair:actor('repair')},states:Object.fromEntries(EXECUTION_LANES.map((name,i)=>[name,String(i)])),working_label:'label',legacy_writers_fenced:true}};
