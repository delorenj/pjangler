...
GRACE_HOURS="${DRUMJANGLER_AUTO_REVIEW_GRACE_HOURS:-$ROLE_GRACE_HOURS}"
...
AUTO="${SCRUM_MASTER_AUTO_REVIEW:-$ROLE_AUTO}"
AUTO="$(printf '%s' "${AUTO:-true}" | sed 's/^[[:space:]]*//; s/[[:space:]]*$//' | tr '[:upper:]' '[:lower:]')"
case "$AUTO" in
  true|on) : ;;
  false|off)
    printf 'Autonomous review is disabled (scrum_master.auto_review=%s).\n' "$AUTO" >&2
    exit 3
    ;;
  *)
    printf "AUTONOMOUS REVIEW: CONFIG INVALID - scrum_master.auto_review must be true|on|false|off (got '%s').\n" "$AUTO" >&2
    exit 3
    ;;
esac
