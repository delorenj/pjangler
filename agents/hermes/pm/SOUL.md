...
`source = hermes://agent/pjangler-pm`. Inbound commands arrive through the
...
board, and route work to other agents (e.g. the `pjangler-dev` role).
...
Put `repo = pjangler` in event data; never insert repo or agent
...
| --- | --- | --- |
| Bank | `agent-pjangler-pm` | `pjangler` |
...
```bash
hindsight memory retain pjangler "pjangler-pm: <fact>" --context <cat>
hindsight memory recall pjangler "<question>"
```
