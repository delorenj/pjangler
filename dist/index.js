...
import { basename as basename7, join as join29, resolve as resolve17 } from "node:path";
...
function createBmadChecks() {
  return [
    ...
    },
    {
      ...
      migrate: (ctx, finding2) => {
        if (finding2.status !== "warn") {
          return {
            id: finding2.id,
            title: finding2.title,
            status: "noop",
            summary: finding2.status === "skip" ? finding2.summary : "BMAD already current",
            changedFiles: [],
            details: []
          };
        }
...
function isVerifiedCanonicalSkillexProjection(path) {
  const normalized = resolve11(path).split(sep6).join("/");
...
`, { mode: 420, flag: "wx" });
  ...
  if (linkExists) {
    ...
    if (target !== realpathSync7(payload) && !isVerifiedCanonicalSkillexProjection(target)) throw new NotebookError("CONFLICT", "Existing Project Notebook skill path is customized or foreign; refusing to replace it");
...
function inspectProjectNotebookIntegration(env2 = process.env) {
  ...
  const link = join20(home, ".agents", "skills", "project-notebook");
  try {
    const expected = expectedPackedSkill();
    const dataRoot = resolve11(env2.XDG_DATA_HOME || join20(home, ".local", "share"), "pjangler", "skills", "project-notebook");
...
function renderDescribe(description, options = {}) {
  ...
  lines.push(field("registry", `${registryNote}  ${dim(shortenPath(identity2.registryPath, options.home))}`));
...
function deriveProjectDefaults(targetDir) {
  ...
  const name = String(manifest?.project_name ?? "").trim() || packageNameToProjectName(typeof pkg?.name === "string" ? pkg.name : void 0) || packageNameToProjectName(basename7(targetDir)) || "Project";
...
async function resolveProjectInitTarget(name, options) {
  ...
  if (!targetDir && interactive) {
    const defaultName = name ?? basename7(cwd);
  ...
  const defaults = targetExists ? deriveProjectDefaults(targetDir) : { name: packageNameToProjectName(basename7(targetDir)) ?? "Project", description: "" };
