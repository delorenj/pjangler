...
import { existsSync as existsSync4, lstatSync as lstatSync4, readFileSync as readFileSync5, readdirSync as readdirSync4, readlinkSync as readlinkSync2 } from "node:fs";
...
function snapshotTree(root, current = root, snapshot = /* @__PURE__ */ new Map()) {
  ...
  if (stat.isSymbolicLink()) {
    ...
    for (const name of readdirSync4(current)) snapshotTree(root, join6(current, name), snapshot);
  } else {
...
function synchronizeCopierIdentity(manifestPath, manifest) {
  ...
  const current = readFileSync7(answersPath, "utf8");
...
function loadProjectRegistry(path = projectRegistryPath()) {
  ...
  const raw = YAML3.parse(readFileSync7(path, "utf8"));
...
`;
  if (!existsSync6(manifestPath) || readFileSync7(manifestPath, "utf8") !== text3) {
  ...
  for (const action of plan.actions) {
    if (action.kind === "copier.copy.commonproject") {
      ...
      if (existsSync6(action.path)) {
        try {
          const currentValue = JSON.parse(readFileSync7(action.path, "utf8"));
...
`;
      const current = existsSync6(action.path) ? readFileSync7(action.path, "utf8") : void 0;
      if (current !== next) {
...
import { closeSync as closeSync4, existsSync as existsSync14, fchmodSync as fchmodSync2, fsyncSync as fsyncSync2, lstatSync as lstatSync8, openSync as openSync4, readFileSync as readFileSync13, realpathSync as realpathSync5, renameSync as renameSync4, writeFileSync as writeFileSync8 } from "node:fs";
...
function persistProjectNotebookBinding(resolved, binding, policy) {
  ...
  const manifestRaw = existsSync14(manifestPath) ? JSON.parse(readFileSync13(manifestPath, "utf8")) : {};
...
`;
  if (!existsSync14(manifestPath) || readFileSync13(manifestPath, "utf8") !== manifestText) {
    let mode = 420;
    ...
    const fd = openSync4(temp, "wx", mode);
    try {
    ...
    try {
      const directory = openSync4(dirname7(manifestPath), "r");
      try {
        fsyncSync2(directory);
      } finally {
        closeSync4(directory);
      }
    } catch {
    }
  ...
  const registryBefore = existsSync14(resolved.registry_path) ? readFileSync13(resolved.registry_path, "utf8") : null;
  saveProjectRegistry(resolved.registry, resolved.registry_path);
  const registryAfter = existsSync14(resolved.registry_path) ? readFileSync13(resolved.registry_path, "utf8") : null;
...
function projectStateDir(root, projectSlug) {
  ...
  return join17(resolve8(root), "projects", sha256Hex(`pjangler-project-state-v1\0${projectSlug}`));
}
...
function openPinnedDirectory(path, root, create) {
  ...
  let fd = openSync5(parsed.root, DIRECTORY_OPEN_FLAGS);
  ...
  try {
    for (const part of absolute.slice(parsed.root.length).split(sep3).filter(Boolean)) {
      ...
      let childFd;
      try {
        childFd = openSync5(child, DIRECTORY_OPEN_FLAGS);
      } catch (error) {
...
function notebookStatePaths(root, projectSlug) {
  ...
  return {
    root: absoluteRoot,
    project,
    baselines: join17(project, "baselines"),
    claims: join17(project, "claims"),
    receipts: join17(project, "receipts"),
    refusals: join17(project, "refusals"),
    journals: join17(project, "journals"),
    locks: join17(project, "locks")
  };
}
function ensureNotebookState(root, projectSlug) {
  ...
  for (const path of [absoluteRoot, join17(absoluteRoot, "projects"), project, paths.baselines, paths.claims, paths.receipts, paths.refusals, paths.journals, paths.locks]) {
...
function readStateDirectory(path, root) {
  ...
  try {
    return readdirSync8(`/proc/self/fd/${fd}`, { withFileTypes: true });
  } finally {
    closeSync5(fd);
  }
}
...
function unlinkStateFile(path, root, allowMissing = false) {
  ...
  try {
    try {
      fileFd = openSync5(target, constants4.O_RDONLY | (constants4.O_NOFOLLOW ?? 0));
    } catch (error) {
      if (allowMissing && error.code === "ENOENT") return false;
      throw error;
    }
    const stat = fstatSync3(fileFd);
    ...
    if (fileFd !== void 0) closeSync5(fileFd);
    closeSync5(parentFd);
  }
}
function renameStateFile(source, target, root) {
  ...
  let sourceFd;
  try {
    sourceFd = openSync5(sourcePath, constants4.O_RDONLY | (constants4.O_NOFOLLOW ?? 0));
    const stat = fstatSync3(sourceFd);
    ...
    if (sourceFd !== void 0) closeSync5(sourceFd);
    closeSync5(parentFd);
  }
}
function atomicWriteJson(path, value, root, afterParentPinned) {
  ...
  try {
    afterParentPinned?.();
    try {
      const existingFd = openSync5(target, constants4.O_RDONLY | (constants4.O_NOFOLLOW ?? 0));
      try {
        const existing = fstatSync3(existingFd);
        ...
        closeSync5(existingFd);
      }
    } catch (error) {
      if (error.code !== "ENOENT") throw error;
    }
    const fd = openSync5(temp, constants4.O_WRONLY | constants4.O_CREAT | constants4.O_EXCL | (constants4.O_NOFOLLOW ?? 0), 384);
    try {
...
function exclusiveWrite(path, text3, root) {
  ...
  let fd;
  try {
    fd = openSync5(target, constants4.O_WRONLY | constants4.O_CREAT | constants4.O_EXCL | (constants4.O_NOFOLLOW ?? 0), 384);
  } catch (error) {
    closeSync5(parentFd);
...
function safeReadJson(path, maxBytes) {
  ...
  let fd;
  try {
    fd = openSync5(pinnedLeaf(parentFd, path), constants4.O_RDONLY | (constants4.O_NOFOLLOW ?? 0));
  } catch (error) {
    closeSync5(parentFd);
...
function createSessionBaseline(root, input) {
  ...
  const path = join17(paths.baselines, `${input.session_key}.json`);
...
function createOverviewClaim(root, input) {
  assertDigest(input.session_key, "session key");
  const paths = ensureNotebookState(root, input.project_slug);
  const path = join17(paths.claims, `${input.session_key}.overview`);
...
function readOverviewClaim(root, projectSlug, sessionKey) {
  assertDigest(sessionKey, "session key");
  const paths = notebookStatePaths(root, projectSlug);
  const path = join17(paths.claims, `${sessionKey}.overview`);
  if (!existsSync15(path)) return null;
  const read = safeReadJson(path, 65536);
...
function readSessionBaseline(root, projectSlug, sessionKey, limits) {
  ...
  return readBaseline(join17(paths.baselines, `${sessionKey}.json`), baselineReceiptByteCeiling(limits));
}
function acquireLock(paths, maxWaitMs) {
  const lock = join17(paths.locks, "admission.lock");
  ...
  while (true) {
    ...
    if (Date.now() >= expiresAt) {
      const recovery = join17(paths.locks, `recovery-${heldToken}.lock`);
      ...
      if (exclusiveWrite(recovery, recoveryRecord, paths.root)) {
        try {
          ...
          if (currentToken === heldToken && Number.isFinite(currentExpiry) && Date.now() >= currentExpiry) {
            const recovered = join17(paths.locks, `.recovered-${heldToken}-${randomUUID3()}.json`);
...
function scanAuxiliaryState(paths, limits) {
  ...
  for (const specification of specifications) {
    ...
    for (const entry of entries) {
      ...
      const read = safeReadJson(join17(specification.dir, entry.name), specification.maxBytes);
  ...
  if (existsSync15(paths.journals)) {
    ...
    for (const entry of entries) {
      ...
      const read = safeReadJson(join17(paths.journals, entry.name), limits.receipt_max_bytes);
      if (read.reason || read.value === void 0) {
        addBoundedIntegrity(scan, limits, { entry_id: entryId, reason: read.reason ?? "invalid-json" }, read.bytes);
        continue;
      }
  ...
  if (existsSync15(paths.locks)) {
    ...
    for (const entry of entries) {
      ...
      const read = safeReadJson(join17(paths.locks, entry.name), 8192);
      if (read.reason || read.value === void 0) {
        addBoundedIntegrity(scan, limits, { entry_id: entryId, reason: read.reason ?? "invalid-json" }, read.bytes);
        continue;
      }
...
function scanReceipts(paths, limits) {
  ...
  for (const entry of entries) {
    ...
    const path = join17(paths.receipts, entry.name);
...
function scanBaselines(paths, nowMs, limits, referenced) {
  ...
  for (const entry of readStateDirectory(paths.baselines, paths.root)) {
    ...
    const baseline = readBaseline(join17(paths.baselines, entry.name), baselineReceiptByteCeiling(limits));
...
function scanRefusals(paths, nowMs, limits, scan) {
  ...
  for (const entry of entries) {
    if (!entry.isFile() || !entry.name.endsWith(".json")) continue;
    const read = safeReadJson(join17(paths.refusals, entry.name), limits.receipt_max_bytes);
...
function removeValidRefusal(paths, sessionKey, limits) {
  const path = join17(paths.refusals, `${sessionKey}.json`);
...
function admitCaptureReceipt(input) {
  ...
  try {
    const receiptId = deriveReceiptId(input.sessionKey);
    const receiptPath = join17(paths.receipts, `${receiptId}.json`);
    if (existsSync15(receiptPath)) {
    ...
    const baselinePath = join17(paths.baselines, `${input.sessionKey}.json`);
    ...
    if (baseline) {
      ...
      if (expired && !referencedBefore.has(input.sessionKey) && scanBefore.integrityCount === 0 && auxiliaryBefore.integrityCount === 0) {
        ...
        const claimPath = join17(paths.claims, `${input.sessionKey}.overview`);
        unlinkStateFile(claimPath, paths.root, true);
        const refusalPath = join17(paths.refusals, `${input.sessionKey}.json`);
    ...
    if (countBlocked || byteBlocked) {
      ...
      const refusalPath = join17(paths.refusals, `${input.sessionKey}.json`);
      const oldRead = existsSync15(refusalPath) ? safeReadJson(refusalPath, input.limits.receipt_max_bytes) : void 0;
...
function readCaptureReceipt(root, projectSlug, receiptId, limits) {
  ...
  const paths = notebookStatePaths(root, projectSlug);
  const path = join17(paths.receipts, `${receiptId}.json`);
...
function writeReceipt(paths, receipt, limit) {
  ...
  for (let i = 0; i < 8; i++) {
    ...
    if (receipt.serialized_bytes === bytes) {
      atomicWriteJson(join17(paths.receipts, `${receipt.receipt_id}.json`), receipt, paths.root);
      return receipt;
    }
    receipt.serialized_bytes = bytes;
    text3 = jsonLine(receipt);
...
function pruneNotebookState(root, projectSlug, limits, now = /* @__PURE__ */ new Date()) {
  ...
  try {
    ...
    for (const entry of readStateDirectory(paths.baselines, paths.root)) {
      if (!entry.isFile() || !entry.name.endsWith(".json")) continue;
      const path = join17(paths.baselines, entry.name);
...
function statePathForReceipt(root, projectSlug, receiptId) {
  if (!RECEIPT_ID_RE.test(receiptId)) throw new NotebookError("INVALID_INPUT", "Invalid receipt ID");
  return join17(projectStateDir(root, projectSlug), "receipts", `${receiptId}.json`);
}
...
var init_state = __esm({
  "src/notebook/state.ts"() {
    ...
    DIRECTORY_OPEN_FLAGS = constants4.O_RDONLY | (constants4.O_DIRECTORY ?? 0) | (constants4.O_NOFOLLOW ?? 0);
  }
});
...
import { closeSync as closeSync6, constants as constants5, fstatSync as fstatSync4, lstatSync as lstatSync10, openSync as openSync6, readSync as readSync2, realpathSync as realpathSync6 } from "node:fs";
import { extname, join as join19, relative as relative9, resolve as resolve9, sep as sep4 } from "node:path";
...
function readSafeEvidenceText(repoPath, relativePath2, maxBytes) {
  ...
  let fd;
  try {
    fd = openSync6(safe.candidate, constants5.O_RDONLY | (constants5.O_NOFOLLOW ?? 0));
  } catch {
...
function journalPath(root, projectSlug, operationId) {
  ...
  return join20(ensureNotebookState(root, projectSlug).journals, `${operationId}.json`);
}
...
function listRemoteMutationJournals(root, projectSlug) {
  ...
  for (const entry of readNotebookStateDirectory(paths.journals, paths.root)) {
    ...
    const read = readNotebookStateJson(join20(paths.journals, entry.name), paths.root, 65536);
...
import { basename as basename9, join as join31, resolve as resolve18 } from "node:path";
...
function readRegularConfigSnapshot(path, expected) {
...
function assertConfigUnchanged(path, previous) {
  if (!previous) {
...
function openConfigLock(path) {
  ...
  const lockPath = join2(directory, CONFIG_LOCK_NAME);
  let existing;
  try {
  ...
  try {
    descriptor = openSync(
      lockPath,
      constants.O_CREAT | constants.O_RDWR | (constants.O_NOFOLLOW ?? 0) | (constants.O_NONBLOCK ?? 0),
      384
    );
  } catch (error) {
...
function writeTransactionMarker(directory, marker) {
  const nextPath = join2(directory, TRANSACTION_STATE_NEXT);
  const statePath = join2(directory, TRANSACTION_STATE);
  writeFileSync2(nextPath, `${JSON.stringify(marker)}
`, { encoding: "utf8", mode: 384 });
  renameSync(nextPath, statePath);
}
function readTransactionMarker(directory) {
  const parsed = JSON.parse(readFileSync(join2(directory, TRANSACTION_STATE), "utf8"));
  const identityValid = (identity2) => Boolean(
    identity2 && [identity2.dev, identity2.ino, identity2.mode, identity2.mtimeMs].every(Number.isFinite) && typeof identity2.hash === "string" && /^[0-9a-f]{64}$/.test(identity2.hash)
  );
...
function recoverInterruptedConfigTransactions(path, allowMutation) {
  ...
  for (const entry of entries) {
    ...
    const transactionDirectory = join2(directory, entry.name);
    ...
    const candidatePath = join2(transactionDirectory, TRANSACTION_CANDIDATE);
    const operatorPath = join2(transactionDirectory, TRANSACTION_OPERATOR_FILE);
...
function installValidatedConfig(path, next, previous) {
  ...
  const directory = dirname2(path);
  mkdirSync2(directory, { recursive: true });
  const transactionDirectory = mkdtempSync(join2(directory, `${CONFIG_TRANSACTION_PREFIX}${process.pid}-`));
  chmodSync(transactionDirectory, 448);
  const stagedPath = join2(transactionDirectory, TRANSACTION_CANDIDATE);
  const operatorPath = join2(transactionDirectory, TRANSACTION_OPERATOR_FILE);
  ...
  try {
    ...
    writeFileSync2(stagedPath, nextBytes, { mode });
    ...
    marker = {
      ...
      phase: "prepared",
      hadPrevious: previous !== void 0,
      candidate: snapshotIdentity(stagedSnapshot),
      previous: previous ? snapshotIdentity(previous) : void 0
    };
    writeTransactionMarker(transactionDirectory, marker);
  ...
  if (cleanupError) {
    const base = failure ? errorDetail(failure) : "Hermes config was installed and validated";
    throw new Error(
...
// src/parity/rules.ts
init_style();
import { existsSync as existsSync3, lstatSync as lstatSync3, mkdirSync as mkdirSync3, mkdtempSync as mkdtempSync2, readFileSync as readFileSync3, readlinkSync, readdirSync as readdirSync3, realpathSync as realpathSync2, renameSync as renameSync2, rmdirSync, symlinkSync, unlinkSync as unlinkSync2, writeFileSync as writeFileSync3, chmodSync as chmodSync2, copyFileSync, rmSync as rmSync2 } from "node:fs";
...
// src/parity/pack.ts
import { closeSync as closeSync2, constants as constants2, fstatSync as fstatSync2, lstatSync as lstatSync2, openSync as openSync2, readFileSync as readFileSync2, readdirSync as readdirSync2 } from "node:fs";
...
function readRegularFile(path) {
  ...
  try {
    if (!fstatSync2(fd).isFile()) throw new Error(`Pack entry is not a regular file: ${path}`);
...
function selectPackVersion(packDir) {
  const versions = [];
  for (const name of readdirSync2(packDir).sort()) {
...
function packContainerLeaves(containerDir) {
  ...
  const visit = (directory, prefix) => {
    let children;
    try {
      children = readdirSync2(directory).sort();
    } catch {
      return;
    }
...
function packDeclaredSkills(root, metadata, entry, flatten = false) {
  ...
  const declared = [];
  for (const name of readdirSync2(root).sort()) {
...
function provisionDeclaredPacks(ctx, preservedManifest, hooks = {}) {
  ...
  const rollback = () => {
    ...
    try {
      if (!skillsExisted && existsSync3(skillsDir) && readdirSync3(skillsDir).length === 0) rmdirSync(skillsDir);
      if (!agentsExisted && existsSync3(safeDirs.agentsDir) && readdirSync3(safeDirs.agentsDir).length === 0) rmdirSync(safeDirs.agentsDir);
    } catch (error) {
...
function bmadProjectNameIssues(repoRoot) {
  ...
  if (existsSync3(bmadRoot)) {
    ...
    try {
      ...
      declared = readdirSync3(bmadRoot);
    }
  ...
  for (const pattern of patterns) {
    if (pattern.includes("*")) {
      ...
      for (const entry of readdirSync3(base, { withFileTypes: true })) {
        ...
        for (const sibling of readdirSync3(parent, { withFileTypes: true })) {
...
`;
        ...
        if (safeReadText(materializePath) !== expectedMaterializer || existsSync3(materializePath) && (lstatSync3(materializePath).mode & 73) === 0) {
          ...
          if (!ctx.dryRun) {
            ...
            chmodSync2(materializePath, 493);
          }
        }
...
function supportedCliProjectionIssues(repoRoot) {
  ...
  for (const rootName of SUPPORTED_CLI_ROOTS) {
    ...
    const hasGeneratedSkill = existsSync3(projectedSkills) && readdirSync3(projectedSkills).some((name) => {
...
import { lstatSync as lstatSync7, readdirSync as readdirSync6 } from "node:fs";
...
var ValidateHermesOptions = class extends Command {
  async invoke() {
    ...
    const roleDir = resolveContainedPath(
      ctx.targetDir,
      join9(ctx.targetDir, "agents", "hermes", role),
...
import { join as join10, dirname as dirname6, relative as relative7 } from "node:path";
import { existsSync as existsSync7, mkdirSync as mkdirSync5, readFileSync as readFileSync8, writeFileSync as writeFileSync5 } from "node:fs";
...
function registerRenderedAgent(ctx, roleDir, role) {
  ...
  const current = readFileSync8(manifestPath, "utf8");
...
function resolveVendoredTemplate(name) {
  ...
  for (let i = 0; i < 8; i++) {
    ...
    if (existsSync7(join10(candidate, "copier.yml"))) return candidate;
...
var RunCopierTemplate = class extends Command {
  async invoke() {
    ...
    if (existsSync7(join10(roleDir, "role.yaml")) && !ctx.force) {
    ...
    const templateSrc = process.env.PJANGLER_HERMES_TEMPLATE || vendored || (existsSync7(join10(LOCAL_TEMPLATE, "copier.yml")) ? LOCAL_TEMPLATE : HERMES_AGENT_TEMPLATE);
    const args = [
      "copy",
      templateSrc,
      roleDir,
...
import { existsSync as existsSync8, readFileSync as readFileSync9, writeFileSync as writeFileSync6, readdirSync as readdirSync7 } from "fs";
...
var UntrackHermesRuntimes = class extends Command {
  async invoke() {
    ...
    const roles = readdirSync7(rolesDir, { withFileTypes: true }).filter((entry) => entry.isDirectory()).map((entry) => entry.name);
    if (roles.length === 0) {
      return {
        success: true,
    ...
    for (const role of roles) {
      ...
      if (existsSync8(gitmodulesPath)) {
        gitmodulesContent = readFileSync9(gitmodulesPath, "utf8");
      ...
      if (existsSync8(fullGitignorePath)) {
        const content = readFileSync9(fullGitignorePath, "utf8");
        const lines = content.split(/\r?\n/).map((line) => line.trim());
        isIgnored = lines.includes("runtime/") || lines.includes("runtime");
      }
...
` : "", "utf8");
        ...
        if (!isIgnored) {
          ...
          if (!this.context.dryRun) {
            let content = "";
            if (existsSync8(fullGitignorePath)) {
              content = readFileSync9(fullGitignorePath, "utf8");
            }
            if (content && !content.endsWith("\n")) {
              content += "\n";
            }
...
function readServiceState(roleDir) {
  if (!roleDir || !existsSync10(join13(roleDir, "role.yaml"))) return { gateway: "planned", heartbeat: "planned" };
  try {
    const role = YAML5.parse(readFileSync10(join13(roleDir, "role.yaml"), "utf8"));
    return {
...
function renderHermesSummary(ctx) {
  ...
  const lines = [
    ...
    `role_dir: ${ctx.roleDir ?? join13(ctx.targetDir, "agents", "hermes", ctx.role ?? "pm")}`,
    `runtime: local role runtime (${join13(ctx.roleDir ?? join13(ctx.targetDir, "agents", "hermes", ctx.role ?? "pm"), "runtime")})`,
...
import { existsSync as existsSync11, readFileSync as readFileSync11, writeFileSync as writeFileSync7 } from "node:fs";
...
function deploymentDeferrals(ctx) {
  ...
  const projectManifest = join16(ctx.targetDir, ".project.json");
  if (!ctx.skipPlane && existsSync13(projectManifest)) {
    try {
      const project = JSON.parse(readFileSync12(projectManifest, "utf8"));
  ...
  const rolePath = ctx.roleDir ? join16(ctx.roleDir, "role.yaml") : "";
  if (rolePath && existsSync13(rolePath)) {
    try {
      const role = YAML7.parse(readFileSync12(rolePath, "utf8"));
...
function manifestNotebook(repo) {
  const path = join18(repo, ".project.json");
...
var PJANGLER_VERSION = (() => {
  try {
    ...
    for (let i = 0; i < 4; i++) {
      try {
        const raw = readFileSync16(join21(dir, "package.json"), "utf8");
...
var DEFAULT_RUNTIME = {
  ...
  spawnWorker(receiptId, projectSlug) {
    ...
    if (!entry) return;
    const child = spawn2(process.execPath, [entry, "notebook", "worker", "capture", "--receipt-id", receiptId], {
      detached: true,
      stdio: "ignore",
...
function enumerateSkillPayload(source) {
  const result2 = [];
  const walk = (directory) => {
    for (const entry of readdirSync9(directory, { withFileTypes: true }).sort((a, b) => a.name.localeCompare(b.name, "en"))) {
      const path = join22(directory, entry.name);
  ...
  if (!existsSync17(payload)) {
    ...
    try {
      for (const entry of manifest.files) {
        ...
        chmodSync5(destination, mode);
        const fd = openSync7(destination, constants6.O_RDONLY | (constants6.O_NOFOLLOW ?? 0));
        try {
          const current = fstatSync5(fd);
...
function repairProjectNotebookSkillProjection(input = {}) {
  ...
  for (const rel of changed) {
    ...
    chmodSync5(destination, entry.mode === "0755" ? 493 : 420);
  }
...
function readHookPayload(input) {
  ...
  const fd = openSync7(path, constants6.O_RDONLY | (constants6.O_NOFOLLOW ?? 0));
  try {
...
`;
    this.writeFile(filePath, content);
    if (!this.context.dryRun) {
      chmodSync6(join26(this.context.targetDir, filePath), 493);
    }
    return {
      success: true,
...
function resolveTemplateConfigPath2(env2 = process.env, home = homedir10()) {
  ...
  const base = xdg && xdg.length ? xdg : join28(home, ".config");
  return join28(base, "hermes-agent-template", "config.toml");
}
...
function readTemplateConfig(env2, home) {
  try {
    ...
    return existsSync22(path) ? readFileSync21(path, "utf8") : void 0;
  } catch {
    return void 0;
  }
}
...
function findProjectRoot(from) {
  ...
  for (; ; ) {
    if (existsSync22(join28(dir, ".project.json"))) return dir;
...
function readTicketProvider(root) {
  try {
    const manifest = JSON.parse(readFileSync21(join28(root, ".project.json"), "utf8"));
  ...
  try {
    ...
    for (; ; ) {
      ...
      if (existsSync22(dotgit)) {
        ...
        const head = readFileSync21(join28(gitDir, "HEAD"), "utf8").trim();
...
import { existsSync as existsSync23, readFileSync as readFileSync22, readdirSync as readdirSync10, statSync as statSync5 } from "node:fs";
import { join as join30, resolve as resolve16 } from "node:path";
init_project();

// src/describe/activity.ts
import { spawn as spawn4, spawnSync as spawnSync14 } from "node:child_process";
...
function gitAsync(repo, args) {
  return new Promise((resolve19) => {
    const child = spawn4("git", ["-C", repo, ...args], { stdio: ["ignore", "pipe", "ignore"] });
    let out = "";
    let size = 0;
    let settled = false;
    const finish = (value) => {
...
function describeType(repo) {
  ...
  for (const marker of LANGUAGE_MARKERS) {
    if (!existsSync23(join30(repo, marker.file))) continue;
  ...
  try {
    const dotnet = readdirSync10(repo).find((entry) => entry.endsWith(".csproj") || entry.endsWith(".sln"));
  ...
  if (pkg) {
    if (existsSync23(join30(repo, "tsconfig.json"))) {
  ...
  for (const [role, marker] of roleMarkers) {
    if (!existsSync23(join30(repo, marker))) continue;
    roles.push(role);
    note(role, marker);
  }
...
function describeSubsystems(repo, findings) {
  ...
  return recipeRegistry.list().map((metadata) => {
    ...
    const evidence = metadata.id === "notebook" ? (() => {
      try {
        const manifest = JSON.parse(readFileSync22(join30(repo, ".project.json"), "utf8"));
        return manifest.notebook && typeof manifest.notebook === "object" ? [".project.json#notebook"] : [];
      } catch {
        return [];
      }
    })() : markers.filter((marker) => existsSync23(join30(repo, marker)));
...
function describeNotebook(repo, registryPath2) {
  try {
    ...
    const manifest = existsSync23(join30(repo, ".project.json")) ? JSON.parse(readFileSync22(join30(repo, ".project.json"), "utf8")) : void 0;
...
function describeConfigFiles(repo) {
  return CONFIG_FILES.filter((spec) => existsSync23(join30(repo, spec.path))).map((spec) => ({ path: spec.path, purpose: spec.purpose, subsystem: spec.subsystem }));
}
...
init_project();
init_capture();
import { closeSync as closeSync8, constants as constants7, existsSync as existsSync24, fstatSync as fstatSync6, lstatSync as lstatSync13, openSync as openSync8, readSync as readSync4 } from "node:fs";
...
function readBoundedRegularFile(path, maxBytes) {
  ...
  const fd = openSync8(absolute, constants7.O_RDONLY | (constants7.O_NOFOLLOW ?? 0));
  try {
...
function deriveProjectDefaults(targetDir) {
  const manifest = readJson2(join31(targetDir, ".project.json"));
  const pkg = readJson2(join31(targetDir, "package.json"));
  const name = String(manifest?.project_name ?? "").trim() || packageNameToProjectName(typeof pkg?.name === "string" ? pkg.name : void 0) || packageNameToProjectName(basename9(targetDir)) || "Project";
...
`;
    return !existsSync25(action.path) || readFileSync23(action.path, "utf8") !== next;
  }
...
async function resolveProjectInitTarget(name, options) {
  ...
  if (!targetDir && interactive) {
    const defaultName = name ?? basename9(cwd);
    const promptedName = name ?? await promptTextValue("Project name", packageNameToProjectName(defaultName));
    const defaultDir = join31(cwd, promptedName.replace(/[^A-Za-z0-9._-]/g, "") || promptedName.toLowerCase().replace(/[^a-z0-9]+/g, "-"));
  ...
  const defaults = targetExists ? deriveProjectDefaults(targetDir) : { name: packageNameToProjectName(basename9(targetDir)) ?? "Project", description: "" };
