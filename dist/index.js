    ...
    audit: (ctx) => {
      ...
      if (existsSync10(legacyDir)) {
        for (const name of readdirSync3(legacyDir)) {
          ...
          try {
            if (!expected || !lstatSync2(path).isSymbolicLink() || resolve3(dirname6(path), readlinkSync(path)) !== expected) invalidBmadLinks++;
          } catch {
            invalidBmadLinks++;
          }
        }
