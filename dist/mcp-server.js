...
function packDeclaredSkills(root, metadata, entry) {
  ...
  for (const name of readdirSync(root).sort()) {
    ...
    if (!isRegularFile(join5(root, name, "SKILL.md"))) continue;
...
function packPayload(root, metadata, declared) {
  ...
  for (const name of declared) {
    ...
    if (!isRegularFile(join5(skillDir, "SKILL.md"))) {
...
function buildPackPlan(ctx, manifest) {
  ...
  if (!plan.bmadDeclared) {
    ...
    try {
      ...
      for (const name of pack.members) {
        const target = join6(root, validateSkillName(name));
  ...
  for (const entry of entries) {
    try {
      ...
      for (const name of pack.members) {
        plan.projections.set(name, join6(root, validateSkillName(name)));
      }
    } catch (error) {
