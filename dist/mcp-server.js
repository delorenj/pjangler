...
`;
  if (!existsSync8(manifestPath) || readFileSync5(manifestPath, "utf8") !== text2) {
...
function isPackManagedManifestEntry(entry, expectedNames, packRoot) {
  ...
  try {
    const sourcePath = resolve3(fileURLToPath5(source));
...
function provisionBmadSkills(ctx, preservedManifest, hooks = {}) {
  ...
  const expected = new Map(packSkills.map((entry) => [entry.name, fileURLToPath5(entry.source)]));
  ...
  const transaction = mkdtempSync2(join15(safeDirs.agentsDir, ".bmad-transaction-"));
...
var RULES = [
  ...
  },
  {
    ...
    audit: (ctx) => {
      ...
      const expectedByName = new Map(expectedBmad.map((entry) => [entry.name, fileURLToPath5(entry.source)]));
