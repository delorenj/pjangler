...
import { existsSync as existsSync6, mkdirSync as mkdirSync4, statSync } from "node:fs";
...
var Command = class {
  ...
  fileExists(filePath) {
    const { existsSync: existsSync7 } = __require("fs");
    const { join: join9 } = __require("path");
    const fullPath = join9(this.context.targetDir, filePath);
  ...
  writeFile(filePath, content) {
    ...
    const { writeFileSync: writeFileSync3, mkdirSync: mkdirSync5 } = __require("fs");
    const { join: join9, dirname: dirname6 } = __require("path");
    const fullPath = join9(this.context.targetDir, filePath);
  ...
  createDirectory(dirPath) {
    ...
    const { join: join9 } = __require("path");
    const fullPath = join9(this.context.targetDir, dirPath);
...
// src/parity/index.ts
import { existsSync as existsSync5, lstatSync, mkdirSync as mkdirSync3, readFileSync as readFileSync3, readlinkSync, readdirSync, renameSync, symlinkSync, unlinkSync as unlinkSync3, writeFileSync as writeFileSync2, chmodSync, copyFileSync } from "node:fs";
...
function resolvePjanglerRoot() {
  ...
  while (dir !== dirname4(dir)) {
    if (existsSync5(join7(dir, "package.json")) && existsSync5(join7(dir, "templates", "commonproject", "copier.yml"))) {
      return dir;
    }
    dir = dirname4(dir);
  }
...
function bootstrapAgentsFile(repoRoot, dryRun) {
  const agentsPath = join7(repoRoot, "AGENTS.md");
  if (existsSync5(agentsPath)) return { changedFiles: [], details: [] };
  for (const file of ["CLAUDE.md", "GEMINI.md"]) {
    const source = join7(repoRoot, file);
    if (!existsSync5(source)) continue;
  ...
  const readmePath = join7(repoRoot, "README.md");
...
function templateScript(ctx, name) {
  const source = join7(ctx.pjanglerRoot, ".mise", "scripts", name);
...
function requiredMisePathEntries(ctx) {
  ...
  for (const candidate of CONDITIONAL_HERMES_PATHS) {
    if (existsSync5(join7(ctx.repoRoot, candidate)) && !required.includes(candidate)) required.push(candidate);
  }
  return required;
}
...
var RULES = [
  {
    ...
    audit: (ctx) => {
      ...
      const details = [];
      const linkAgentfilesPath = join7(ctx.repoRoot, ".mise", "scripts", "link-agentfiles.sh");
      if (!existsSync5(linkAgentfilesPath)) details.push(".mise/scripts/link-agentfiles.sh missing");
  ...
  },
  {
    ...
    audit: (ctx) => {
      ...
      if (!existsSync5(agentsPath)) {
        const fallbackSources = ["CLAUDE.md", "GEMINI.md", "README.md"].filter((file) => existsSync5(join7(ctx.repoRoot, file)));
  ...
  },
  {
    ...
    migrate: (ctx, finding) => {
      ...
      if (!existsSync5(envOpPath)) {
        ...
        if (!ctx.dryRun) writeText(envOpPath, readText(join7(ctx.pjanglerRoot, "templates", "commonproject", "template", ".env.op")));
      }
...
`;
  ...
  },
  {
    ...
    audit: (ctx) => {
      ...
      const missing = sentinels.filter((file) => existsSync5(join7(sourceRoot, file)) && !existsSync5(join7(targetRoot, file)));
      return {
        id: "bmad.scaffold",
        title: "BMAD modules/docs scaffold",
  ...
  },
  {
    ...
    audit: (ctx) => {
      ...
      for (const rel of ["role.yaml", "SOUL.md", "hermes", ".gitignore", ".scripts/70-systemd.sh", ".runtime-scaffold/README.md", "runtime/memories/MEMORY.md", "runtime/bloodbank-consumer.py"]) {
        if (!existsSync5(join7(role.roleDir, rel))) details.push(`missing ${relative(ctx.repoRoot, join7(role.roleDir, rel))}`);
      }
  ...
  },
  {
    ...
    migrate: (ctx, finding) => {
      ...
      for (const role of roles) {
        ...
        const allUnitsPresent = units.every((unit) => existsSync5(join7(sysDir, unit)));
        if (allUnitsPresent) {
          if (ctx.dryRun) {
...
server.registerTool(
  ...
  async (input) => {
    try {
      ...
      if (!existsSync6(parentDir) || !statSync(parentDir).isDirectory()) throw new Error(`Parent directory does not exist: ${parentDir}`);
      const targetDir = resolve2(input.targetDir ?? join8(parentDir, projectSlug));
      ...
      if (existsSync6(targetDir) && !overwrite) throw new Error(`Target already exists: ${targetDir} (set force/overwrite=true to re-render)`);
      const templateDir = join8(pjanglerRoot, "templates", "commonproject");
      ...
      const actions = [
        { kind: "ensure.parent", path: dirname5(targetDir) },
        copierAction
      ];
      ...
      mkdirSync4(dirname5(targetDir), { recursive: true });
