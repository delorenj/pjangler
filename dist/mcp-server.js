...
import { basename as basename5, dirname as dirname9, join as join18, resolve as resolve8 } from "node:path";
...
import { existsSync as existsSync13, readFileSync as readFileSync11, readdirSync as readdirSync5, statSync as statSync2 } from "node:fs";
...
function resolvePjanglerRoot3() {
  ...
  while (dir !== dirname9(dir)) {
    if (existsSync14(join18(dir, "package.json")) && existsSync14(join18(dir, "templates", "commonproject", "copier.yml"))) {
      return dir;
    }
    dir = dirname9(dir);
  }
...
server.registerTool(
  ...
  async (input) => {
    try {
      ...
      if (!existsSync14(parentDir) || !statSync3(parentDir).isDirectory()) throw new Error(`Parent directory does not exist: ${parentDir}`);
      const targetDir = resolve8(input.targetDir ?? join18(parentDir, projectSlug));
      const overwrite = input.overwrite ?? input.force ?? false;
